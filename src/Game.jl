...
function mainloop(g::Game)
  ...
  while true
    ...
    tick!(scheduler[])
    playing[] == false && throw(QuitException())
  end
end
...
function handle_mouse_click(g::Game, e, t)
  ...
  if t == SDL2.MOUSEBUTTONUP
    Base.invokelatest(
        g.onmouseup_function, g,
        (x, y),
        MouseButtons.MouseButton(button)
       )
  elseif t == SDL2.MOUSEBUTTONDOWN
    Base.invokelatest(
        g.onmousedown_function, g,
        (x, y),
        MouseButtons.MouseButton(button)
       )
  end
...
# pause the interpreter. For release builds, the catch() block will call
# quitSDL().
...
function initSDL()
  ...
  r != 0 && error("Uanble to initialise SDL: $(get_SDL_error())")
  SDL2.TTF_Init()
